...
![八个工作切面的前列项目](boards.png)

100 个已完成 13 节架构分析的自动化科研开源项目。同一套公开快照打四项分，再按工作对象分成八个切面，每个切面列出前列。

- 公开站：<https://research-harness.github.io/AutoResearch-Hot100/>
...
- 公开切面：8 个，每个切面最多 10 名
- 每篇分析：固定 13 节、至少一张架构图、固定 commit、公开证据文件清单与事实边界

先按切面看名次，再点项目名进入独立分析页。同一批分析在 [Inside Agentic Science](https://research-harness.github.io/Inside-Agentic-Science/) 里按运行对象切开：一章只讲一个机制（状态机、证据对象、内环、沙箱等）。每章末尾列出实现过该模块的开源项目，接到仓库内部路径和冻结 commit。

## 评分口径

榜单不是运行效果实测，也不只按 stars 排序。每个已发布项目用同一套公开快照打四项分，每项 0–25，加总后在切面内取前列，最多 10 名。
